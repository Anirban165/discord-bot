...
let pool = [];
const checkBlacklistUsers = (channel, blacklist) => {
    if (!channel)
        return false;
...
exports.default = async (client, oldState, newState) => {
    ...
    if (newState.channelId === null) {
        ...
        if (display)
            console.log(`-- ${newState.member?.user.username} joined channel ${newState.channel?.name}`);
        // If the member who left the channel is not bot, check if the channel still has members
        if (!oldState.member?.user.bot) {
        ...
        if (display)
            console.log(`-- ${newState.member?.user.username} moved channel ${oldState.channel?.name} to ${newState.channel?.name}`);
        // If the member who left the channel is not bot, check if the channel still has members
        if (!oldState.member?.user.bot) {
            const player = client.lavashark.getPlayer(oldState.guild.id);
            if (!player)
