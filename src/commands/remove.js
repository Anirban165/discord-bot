...
exports.name = 'remove';
exports.aliases = ['rm'];
exports.description = 'Select a song to remove from the playlist';
exports.usage = 'remove <track index number> [from index to index 2 track]';
    ...
    if (args.length === 1) { // +rm 1
        ...
        if (!SUCCESS) {
            ...
            return message.reply({
                embeds: [embeds_1.embeds.removeTrack(client.config.embedsColor, tracks[index - 1])],
                allowedMentions: { repliedUser: false }
            });
        }
    }
    else if (args.length === 2) { // +rm 3 4
        let index1 = parseInt(args[0]), index2 = parseInt(args[1]);
        ...
        if (!SUCCESS) {
            ...
            await message.react('👍');
            return message.reply({
                embeds: [embeds_1.embeds.removeTrack(client.config.embedsColor, musicTitle)],
                allowedMentions: { repliedUser: false }
            });
        }
    }
        ...
        await message.reply({
            content: instruction,
            embeds: [embeds_1.embeds.removeList(client.config.embedsColor, nowplaying, tracksQueue, methods[repeatMode])],
            allowedMentions: { repliedUser: false }
        });
